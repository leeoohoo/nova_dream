...
- **内置版（推荐）**：本仓库的 `npm run desktop:dist` 会在打包前自动执行 `npm run aide:embed`，把 AIDE 引擎（release dist）与内置应用一起打进安装包，首次启动开箱即用。
  - 约定 AIDE 源码目录在 `../aide`；也可通过环境变量 `MODEL_CLI_AIDE_EMBED_ROOT` 或命令行参数 `--aide-root <path>` 指定。
...
```
aide/src/             # CLI 主体、聊天循环、prompt、MCP runtime
aide/subagents/       # 子代理管理、marketplace、插件（python / spring-boot / frontend-react）
aide/mcp_servers/     # Shell（含会话工具）等 MCP 服务器
