...
1) 复制模板插件：`deepseek_cli/ui_apps/template/basic-plugin` → 放到插件目录（见第 2 节）
...
- **内置/开发目录**：`<projectRoot>/ui_apps/plugins`（本仓库：`deepseek_cli/ui_apps/plugins`）
...
1) `deepseek_cli/ui_apps/plugins/<plugin>/plugin.json`：只放开关
...
- UI Apps 插件模板：`deepseek_cli/ui_apps/template/basic-plugin/plugin.json`
- 宿主扫描/同步 AI 贡献：`deepseek_cli/electron/ui-apps/index.js`
