...
- **Bundled (recommended)**: `npm run desktop:dist` runs `npm run aide:embed` before packaging, so the release AIDE runtime + built-in apps ship inside the installer and work out-of-the-box.
  - By default it expects the AIDE repo at `../aide`; override via `MODEL_CLI_AIDE_EMBED_ROOT` or `--aide-root <path>`.
...
```
aide/src/             # CLI, chat loop, prompts, MCP runtime
aide/subagents/       # marketplace + plugins (python, spring-boot, frontend-react)
aide/mcp_servers/     # shell server with session tools, others unchanged
