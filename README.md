...
- **ChatOS（宿主）**：桌面端 Electron 应用（工作区：`deepseek_cli/`），提供对话、Agent、应用中心与管理台。
- **AIDE（引擎）**：模型调用/工具/MCP/子代理等能力（工作区：`aide/`），并以内置应用的形式提供可视化面板。
...
- 左侧栏：
  ...
  - 管理会话：新建、重命名、删除、刷新
- 会话顶部：
  - `选择目录` / 手动输入 `cwd`（该会话的工具/MCP 会以此作为工作目录 root）
  - `清除` 可恢复为默认启动目录
- 消息输入框：
...
- **ChatOS (host)**: Electron desktop app (workspace: `deepseek_cli/`) — chat, agents, apps, and admin panels.
- **AIDE (engine)**: model runtime + tools/MCP/sub-agents (workspace: `aide/`) — also shipped as a built-in UI app.
