...
const UI_FLAGS = { developerMode: UI_DEVELOPER_MODE, aideInstalled: AIDE_INSTALLED };
...
const sanitizeAdminForUi = (snapshot) => {
  ...
  if (UI_DEVELOPER_MODE) return sanitized;
...
const AIDE_NOT_INSTALLED_MESSAGE =
  'AIDE 引擎不可用：请确认当前安装包已内置引擎（必要时重新安装/重新打包）；开发环境请确保存在 ../aide 或 <projectRoot>/aide。';
...
registerAdminApi(ipcMain, adminServices, () => mainWindow, {
  exposeSubagents: UI_DEVELOPER_MODE,
  uiFlags: UI_FLAGS,
  onChange: async () => {
...
ipcMain.handle('subagents:marketplace:list', async () => {
  if (!UI_DEVELOPER_MODE) {
    return { ok: true, marketplace: [], sources: [] };
  }
  try {
    return {
      ok: true,
      marketplace: subAgentManager.listMarketplace(),
      sources: subAgentManager.listMarketplaceSources(),
    };
  } catch (err) {
    return { ok: false, message: err?.message || String(err) };
  }
...
ipcMain.handle('subagents:marketplace:addSource', async (_event, payload = {}) => {
  if (!UI_DEVELOPER_MODE) {
    return { ok: false, message: 'Sub-agents 管理仅在开发者模式下可用。' };
  }
