...
import { buildAdminSeed } from '../shared/data/legacy.js';
import { resolveSessionRoot, persistSessionRoot, createAdminDefaultsManager } from './admin-defaults.js';
...
const APP_DISPLAY_NAME = 'AIDE';
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const projectRoot = path.join(__dirname, '..');
process.env.MODEL_CLI_HOST_APP = 'aide';
const hostApp =
  String(process.env.MODEL_CLI_HOST_APP || 'aide')
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9_-]+/g, '_')
    .replace(/^_+|_+$/g, '') || 'aide';
...
const DEFAULT_CLI_COMMAND_NAME = 'aide';
// Windows 上桌面版通过 WindowsApps 放一个 .cmd，很容易和 npm 全局安装的 `aide` 发生 PATH 冲突；
// 默认改成另一个名字，避免覆盖/抢占用户的终端命令。
const WINDOWS_DESKTOP_CLI_COMMAND_NAME = 'aide-desktop';
...
const UI_FLAGS = {
  developerMode: UI_DEVELOPER_MODE,
  exposeSubagents: UI_EXPOSE_SUBAGENTS,
};
const INSTALL_ALL_SUBAGENTS = resolveBoolEnv('MODEL_CLI_INSTALL_ALL_SUBAGENTS', true);
const ENABLE_ALL_SUBAGENTS = resolveBoolEnv('MODEL_CLI_ENABLE_ALL_SUBAGENTS', false);
...
function maybePurgeUiAppsSyncedAdminData({ stateDir: stateDirArg, adminServices: adminServicesArg } = {}) {
  ...
  if (hostApp !== 'aide') return { removedServers: 0, removedPrompts: 0 };
...
const legacyAdminDbApp = path.join(os.homedir(), '.deepseek_cli', hostApp, `${hostApp}.db.json`);
const legacyAdminDbShared = path.join(os.homedir(), '.deepseek_cli', 'admin.db.json');
const legacyAdminSeed = readLegacyState(legacyAdminDbApp) || readLegacyState(legacyAdminDbShared);
if (INSTALL_ALL_SUBAGENTS) {
...
const adminDb = createDb({
  ...
  seed: legacyAdminSeed || buildAdminSeed(defaultPaths),
});
...
const subAgentManager = createSubAgentManager({
  baseDir: path.join(projectRoot, 'subagents'),
  stateDir,
});
...
const cliShim = createCliShim({ projectRoot, commandName: CLI_COMMAND_NAME });
const legacyCliShim =
  process.platform === 'win32' && CLI_COMMAND_NAME !== LEGACY_CLI_COMMAND_NAME
    ? createCliShim({ projectRoot, commandName: LEGACY_CLI_COMMAND_NAME })
    : null;
...
const uiAppsManager = registerUiAppsApi(ipcMain, { projectRoot, stateDir, adminServices, onAdminMutation: syncAdminAndBroadcast });
