...
import { parseJsonSafe, safeRead } from '../shared/data/legacy.js';
...
export function createTerminalManager({
  ...
  const baseTerminalsDir =
    typeof terminalsDir === 'string' && terminalsDir.trim()
      ...
      : path.join(resolveAppStateDir(baseSessionRoot), 'terminals');
  ...
  async function ensureCliRunning(runId, options = {}) {
    const rid = typeof runId === 'string' ? runId.trim() : '';
    if (!rid) throw new Error('runId is required');
    const status = readTerminalStatus(rid);
    ...
    if (alive) return;
    if (isSystemTerminalLaunchPending(rid)) {
      return;
    }
  ...
  async function dispatchMessage(payload = {}) {
    ...
    if (!status) {
      ...
      if (!status && isSystemTerminalLaunchPending(runId)) {
    ...
    if (!status) {
      ...
      if (legacyAlive) {
        if (created || isSystemTerminalLaunchPending(runId)) {
          return {
            ok: false,
            reason: 'not_ready',
            runId,
            message: '终端/CLI 仍在启动中（尚未检测到状态上报）。请稍候重试；若持续较久仍无状态上报，请重启该终端的 aide。',
          };
        }
        return {
          ok: false,
