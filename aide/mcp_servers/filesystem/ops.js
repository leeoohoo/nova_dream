...
export function createFilesystemOps({
  ...
  function resolvePathWithinWorkspace(rawPath, baseDir = root) {
    ...
    if (isAbs) {
      ...
      const rerooted = path.resolve(root, trimmedAbs);
  ...
  // Strip common LLM wrappers (code fences/indent) and normalize patch text.
  function preprocessPatchText(patchText = '') {
    if (!patchText) return '';
