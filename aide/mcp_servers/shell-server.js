...
async function promptFileChangeConfirm({ title, message, command, cwd, diff, path: filePath, source } = {}) {
  ...
  const promptPayload = {
    ...
    diff: truncateForUi(typeof diff === 'string' ? diff : '', 60_000),
  };
  appendPromptEntry({
  ...
  const runTtyConfirm = async ({ signal } = {}) => {
    ...
    const shownDiff = rawDiff ? truncateForUi(rawDiff, 20_000) : '';
